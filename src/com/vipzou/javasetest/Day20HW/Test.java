...
public class Test {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < split.length; i++) {
            if (i % 3 == 0) {
                s0[size] = new Student(Integer.parseInt(split[i]), split[i + 1], Double.parseDouble(split[i + 2]));
                size++;
            }
        }
        ...
        sort(s0, size);
        System.out.println(find(s0, size));
    ...
    private static boolean find(Student[] s0, int size) {
        for (int i = 0; i < size; i++) {
            if ("xiaoxiao".equals(s0[i].getName())) {
                return true;
            }
        }
        return false;
